    ...
    const handleSubmit: ShortTestFormSubmitCallback = useCallback(
        async (values, { resetForm, setSubmitting, setFieldError }) => {
            const isNoAlreadyInUse =
                shortTests?.find((t) => t.shortTestNo.toString(10) === values.shortTestNo) !==
                undefined;
        ...
        (shortTest) => {
            return async (values, { setSubmitting }) => {
                try {
                    ...
                    setShortTests(
                        (shortTests ?? []).map((s) =>
                            s.id === shortTest.id ? updatedShortTest : s
                        )
                    );
                    enqueueSnackbar(
                        `${updatedShortTest.toDisplayString()} erfolgreich gespeichert.`,
                        {
                            variant: 'success',
                        }
                    );
        ...
        async (shortTest: ShortTest) => {
            ...
            if (isOkay) {
                try {
                    ...
                    logger.error(error);
                    enqueueSnackbar(
                        `${shortTest.toDisplayString()} konnte nicht gelöscht werden.`,
                        {
                            variant: 'error',
                        }
                    );
