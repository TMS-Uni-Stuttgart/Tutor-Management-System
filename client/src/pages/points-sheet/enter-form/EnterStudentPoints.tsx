...
function EnterStudentPoints(): JSX.Element {
    ...
    if (!tutorialId || !sheetId || !studentId || !teamId) {
        return (
            <Typography color='error'>
                At least one of the three required params{' '}
                <code>tutorialId, sheetId, studentId</code> was not provided through path params.
            </Typography>
        );
    }
    ...
    const handleStudentChange = (event: React.ChangeEvent<{ value: unknown }>) => {
        ...
        history.push(
            ROUTES.ENTER_POINTS_STUDENT.create({ tutorialId, sheetId, teamId, studentId })
        );
    };
