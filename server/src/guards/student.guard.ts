 ...
 *
 * By default the `id` param is assumed to be the one belonging to the student. If the student ID is in a different param field one can set that field with the `@IDField()` decorator.
 *
...
export class StudentGuard extends TutorialGuard {
    ...
    private async getStudentFromRequest(context: ExecutionContext): Promise<Student> {
